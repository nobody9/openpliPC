...
static void decode_poc( vdpau_h264_alter_decoder_t *this_gen )
{
    ...
    if ( sp->pic_order_cnt_type == 0 ) {
      ...
      seq->cur_pic.top_field_first = ( sl->delta_pic_order_cnt_bottom < 0 ) ? 0 : 1;
      
      if ( !prev_pic ) {
        seq->cur_pic.PicOrderCntMsb = seq->cur_pic.TopFieldOrderCnt = seq->cur_pic.BottomFieldOrderCnt = 0;
        return;
      }
      ...
      if ( sp->pic_order_cnt_type == 1 ) {
        int16_t absFrameNum=0, picOrderCntCycleCnt=0, frameNumInPicOrderCntCycle=0, expectedDeltaPerPicOrderCntCycle=0, expectedPicOrderCnt=0;
        int i;
        ...
        if ( absFrameNum > 0 ) {
          picOrderCntCycleCnt = ( absFrameNum - 1 ) / sp->num_ref_frames_in_pic_order_cnt_cycle;
          frameNumInPicOrderCntCycle = ( absFrameNum - 1 ) % sp->num_ref_frames_in_pic_order_cnt_cycle;
        ...
        if ( !sl->field_pic_flag ) {
          seq->cur_pic.TopFieldOrderCnt = expectedPicOrderCnt + sl->delta_pic_order_cnt[0];
          seq->cur_pic.BottomFieldOrderCnt = seq->cur_pic.TopFieldOrderCnt + sp->offset_for_top_to_bottom_field + sl->delta_pic_order_cnt[1];
        }
        else if ( !sl->bottom_field_flag )
          seq->cur_pic.TopFieldOrderCnt = expectedPicOrderCnt + sl->delta_pic_order_cnt[0];
        else
          seq->cur_pic.BottomFieldOrderCnt = expectedPicOrderCnt + sp->offset_for_top_to_bottom_field + sl->delta_pic_order_cnt[1];
      }
      else {
        ...
        if ( !sl->field_pic_flag )
          seq->cur_pic.TopFieldOrderCnt = seq->cur_pic.BottomFieldOrderCnt = tmpPicOrderCnt;
        else if ( sl->bottom_field_flag )
